...
const MODULES = [
  {
    ...
    hooks: [
      "You don't need a certificate or 5 years of experience — just the right AI tools and 2 focused hours a day.",
      "The 3 AI tools beginners are quietly using to deliver work that looks senior-level.",
      "The exact way to price your first project so a client says yes without you underselling yourself.",
    ],
    Icon: Sparkles,
    points: [
  ...
  },
  {
    ...
    hooks: [
      "Zero naira spent on stock, zero warehouse — here's the exact model that makes that possible.",
      "The one supplier mistake that quietly kills most first-time stores before they even get a sale.",
      "How much to realistically set aside for your first month so ads don't eat you alive.",
    ],
    Icon: Package,
    points: [
  ...
  },
  {
    ...
    hooks: [
      "One platform, one post a day — the unglamorous routine quietly paying real people right now.",
      "Why chasing a viral moment is the wrong goal, and what actually converts into income.",
      "The honest timeline before this turns into real money — most people quit right before it does.",
    ],
    Icon: Video,
    points: [
  ...
  },
  {
    ...
    hooks: [
      "The slow, unglamorous approach that protects what little you start with while it grows.",
      "The one beginner mistake that wipes out most first-month traders — easy to avoid once you see it.",
      "How to start small enough that a mistake won't hurt you, but big enough to actually matter over time.",
    ],
    Icon: TrendingUp,
    points: [
  ...
  },
  {
    ...
    hooks: [
      "Your next paying customer is probably someone in your own neighborhood you haven't met yet.",
      "The fastest, cheapest way to land a paying job this month — no ads, no website needed.",
      "A pricing method that stops you from underselling your own hands and your own time.",
    ],
    Icon: Wrench,
    points: [
  ...
  },
  {
    ...
    hooks: [
      "Get paid for recommending things you already believe in — no product to build, no inventory to hold.",
      "The one rule that makes people trust your recommendation instead of scrolling past it.",
      "Why a small, focused audience often earns more than a big, scattered one doing this.",
    ],
    Icon: Link2,
    points: [
...
const STEPS = [
  { id: "name", key: "name", input: "text", placeholder: "Your name", bot: () => "Let's unlock your plan — what should I call you?" },
      ...
      {phase === "hook" && (
        <div style={{ padding: "1.5rem 1.25rem", overflowY: "auto" }}>
          <div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
            <div style={{ color: C.coral, fontSize: "0.8rem", fontWeight: 600, marginBottom: "0.5rem" }}>
              The salary runs out before the month does. You're not alone in that.
            </div>
            <div style={{ fontFamily: "'Sora', sans-serif", fontWeight: 800, fontSize: "1.4rem", lineHeight: 1.3 }}>
              6 Real Ways People Are Closing <span style={{ color: C.gold }}>The Gap</span>
            </div>
            <div style={{ color: C.muted, fontSize: "0.85rem", marginTop: "0.5rem" }}>
              The full breakdown for each — free, unlocked in 2 minutes.
            </div>
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: "0.8rem", marginBottom: "1.5rem" }}>
            {MODULES.map((m) => (
              <div key={m.id} style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: "12px", padding: "1rem" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "0.7rem" }}>
                  <div style={{ background: C.panelLight, borderRadius: "8px", padding: "0.5rem", display: "flex" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.45rem", marginBottom: "0.7rem" }}>
                  {m.hooks.map((h, i) => (
                    <div key={i} style={{ display: "flex", gap: "0.5rem", fontSize: "0.82rem", lineHeight: 1.45, color: C.text }}>
                      <Zap size={13} color={C.gold} style={{ flexShrink: 0, marginTop: "3px" }} />
                      <span>{h}</span>
                    </div>
                  ))}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: "0.35rem", fontSize: "0.7rem", color: C.muted, borderTop: `1px solid ${C.line}`, paddingTop: "0.6rem" }}>
                  <Lock size={11} /> Full playbook locked — unlock below
                </div>
              </div>
            ))}
          </div>

          <div style={{ textAlign: "center", color: C.muted, fontSize: "0.82rem", marginBottom: "1rem", lineHeight: 1.5 }}>
            I've got the full playbook for each one, plus the tools to get started this week. A few honest questions, and it's yours.
          </div>

          <button onClick={() => setPhase("chat")} style={primaryBtn}>
            Show Me How →
          </button>
        </div>
      )}
